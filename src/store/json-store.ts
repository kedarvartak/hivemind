...
export class JsonStore implements MemoryStore {
  ...
  private async writeIndex(blocks: MemoryBlock[]): Promise<void> {
    await fs.writeFile(this.indexPath, JSON.stringify(blocks, null, 2), "utf-8");
  }
  ...
  private async persistBlock(block: MemoryBlock): Promise<void> {
    await fs.writeFile(this.blockPath(block.id), JSON.stringify(block, null, 2), "utf-8");
  }
  ...
  async createBlock(topic: string, content = "", tags: string[] = []): Promise<MemoryBlock> {
    ...
    console.error(`[hivemind] Created block "${topic}" (${block.id})`);
    return block;
  }

  async updateBlock(id: string, content: string, tags?: string[], topic?: string): Promise<MemoryBlock> {
    ...
    console.error(`[hivemind] Updated block "${updated.topic}" (${id}) -> v${updated.version}`);
    return updated;
  }

  async appendBlock(id: string, text: string, tags?: string[]): Promise<MemoryBlock> {
    const existing = await this.readBlock(id);
    if (!existing) throw new Error(`Block not found: ${id}`);
  ...
  async deleteBlock(id: string): Promise<void> {
    ...
    console.error(`[hivemind] Deleted block ${id}`);
  }
